...
public class Baekjoon_24060 {
    static int[] A; //입력받은 배열
    static int[] tmp; //정렬 후 저장하는 배열
    static int count = 0; //저장 횟수 누적 저장
    static int K; // 저장 횟수
    ...
    public static void main(String[] args) throws IOException {
        ...
        int N = Integer.parseInt(stringTokenizer.nextToken()); // 배열의 크기
        K = Integer.parseInt(stringTokenizer.nextToken()); // 저장 횟수
    ...
    public static void merge_sort(int[] A, int p, int r) {
        if(count > K) return; //저장 횟수가 진행 횟수보다 넘어가게 되면 더이상 분해 or 병합 진행X
        if(p < r) {
    ...
    */
    public static void merge(int[] A, int p, int q, int r) {
        ...
        while(i <= q && j <= r) {
            if(A[i] <= A[j]) { //만약 Array[i](=앞의 값)이/가 Array[j](=뒤의 값)보다 작을 경우
                tmp[t] = A[i]; //작은 값을 넣어주고
                i++;
            } else { //만약 Array[i](=앞의 값)이/가 Array[j](=뒤의 값)보다 크거나 같을 경우
                tmp[t] = A[j]; //작은 값이 Array[j]이므로 작은 값을 차근차근 쌓는다.
                j++;
            }
            t++;
        }
        ...
        while(i <= r) { //결과를 배열 A에 저장한다. i(시작), r(끝)
            count++;
            if(count == K) { //저장횟수가 같다면
                result = tmp[t]; //해당 증가한 만큼의 t인덱스의 값을 result변수에 담고
                break;
            }
            A[i++] = tmp[t++]; //A에 순서대로 정렬된 값을 저장
        }
    }
}
